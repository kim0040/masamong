...
#### ⚡️ Smart V2.1 (Chunking & Migration Supported)

대화형 Python 스크립트로, 더 쉽고 강력하게 업그레이드되었습니다. 
**V2.1 업데이트**: 이제 긴 세션을 요약하는 것에 그치지 않고, 검색에 최적화된 "의미 단위 청크(Chunk)"로 쪼개어 저장합니다.

**🔥 주요 기능**
1.  **🔪 정밀 검색 (Chunking)**: 요약문(전체 맥락) + 청크(상세 대화)를 결합하여, "그때 그 얘기"와 "구체적인 단어"를 모두 잡아냅니다.
2.  **🚀 5배 빠른 속도**: 병렬 처리(Concurrency=5)로 대량의 대화도 순식간에 처리합니다.
3.  **💾 이어하기 (Resume)**: 작업이 중단되어도 `checkpoint.jsonl`을 통해 자동으로 이어서 진행합니다.
4.  **🔄 마이그레이션 모드**: 기존 V2 데이터가 있다면 요약 비용 없이 청킹만 다시 수행할 수 있습니다. (`--migrate-v2`)
