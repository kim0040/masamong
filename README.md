...
기존 2-Step 아키텍처(Lite → Main)는 다음 문제가 있었습니다:

| 문제 | 설명 |
|-----|-----|
| **API 할당량 소진** | 메시지당 2회 호출로 무료 티어(20회/분) 빠르게 소진 |
| **JSON 파싱 불안정** | Lite 모델이 가끔 잘못된 JSON 출력 → 응답 실패 |
| **응답 지연** | 2번 API 왕복으로 3-5초 소요 |
| **복잡한 디버깅** | 문제 발생 시 Lite vs Main 중 어디서 실패했는지 추적 어려움 |

**해결책**: 키워드 패턴 매칭으로 도구 필요 여부를 0.1ms 내에 판단하여:
- API 호출 **50% 감소** (2회 → 1회)
- 응답 시간 **~1초 단축**
- 파싱 오류 **완전 제거**
