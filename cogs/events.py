...
from .commands import get_current_time
...
class EventListeners(commands.Cog):
    ...
    async def _handle_ai_interaction(self, message: discord.Message):
        ...
        if intent == 'Time':
            logger.debug("시간 질문 의도로 판단됨.")
            current_time_str = get_current_time()
            # AI를 통해 좀 더 자연스러운 답변 생성
            time_info_for_ai = f"현재 한국 시간은 {current_time_str} 입니다."
            await self.ai_handler.process_ai_message(message, weather_info=time_info_for_ai) # weather_info 파라미터를 재활용하여 추가 정보 전달
        elif intent == 'Chat':
