...
class SettingsCog(commands.Cog):
    ...
    async def set_persona(self, interaction: discord.Interaction, new_persona: str):
        """서버의 AI 페르소나를 데이터베이스에 저장/업데이트합니다."""
        if not self.bot.db:
            await interaction.response.send_message("오류: 데이터베이스가 준비되지 않았습니다.", ephemeral=True)
            return

        guild_id = interaction.guild_id
        await interaction.response.defer(ephemeral=True) # 응답 대기

        try:
            sql = """
                INSERT INTO guild_settings (guild_id, setting_name, value)
                VALUES (?, 'persona', ?)
            ...
            logger.info(f"서버({guild_id}) 페르소나 업데이트됨. 내용: {new_persona[:50]}...")

            embed = discord.Embed(
                title="✅ 페르소나 설정 완료",
                description=f"이제부터 이 서버에서 마사몽은 아래 페르소나에 따라 행동합니다.",
            ...
            await interaction.followup.send("오류: 페르소나를 설정하는 중에 문제가 발생했습니다. 다시 시도해주세요.")
    ...
    async def view_persona(self, interaction: discord.Interaction):
        """데이터베이스에서 현재 서버의 페르소나를 조회하여 보여줍니다."""
        if not self.bot.db:
            await interaction.response.send_message("오류: 데이터베이스가 준비되지 않았습니다.", ephemeral=True)
            return
        ...
        try:
            ...
            if result and result[0]:
                persona = result[0]
                embed = discord.Embed(
                    title=f"📜 {interaction.guild.name} 서버의 현재 페르소나",
                    description=persona,
                    color=discord.Color.blue()
                )
            else:
                embed = discord.Embed(
                    title="ℹ️ 커스텀 페르소나 없음",
                    description="이 서버에는 커스텀 페르소나가 설정되지 않았습니다. 기본 페르소나로 작동하고 있습니다.\n`/persona set` 명령어로 새로운 페르소나를 설정할 수 있습니다.",
            ...
            await interaction.followup.send("오류: 페르소나를 조회하는 중에 문제가 발생했습니다. 다시 시도해주세요.")
...
async def setup(bot: commands.Bot):
    ...
    # 슬래시 커맨드를 특정 길드에만 즉시 등록하려면 아래 주석을 해제
