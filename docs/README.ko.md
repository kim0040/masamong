...
- `!이미지 <프롬프트>`: `gemini-3.1-flash-lite-image`와 관련 기억을 사용하며
  사용자/서버/전역 사용량 제한을 확인한 이미지 생성
- `!랭킹`: 서버 활동 순위와 차트
...
상세 2건, 총 34건의 본문·첨부 링크를 검증했다. 16개 source는 정상, 한양대 서울
source 한 곳은 확인한 공지 중 하나가 이미지 본문만 제공해 `degraded`였다. 목록·상세
URL이 모두 열린 경우라도 이미지 안의 내용은 추측하지 않고 공식 원문 확인을 안내한다.
