...
- 한 검색 안에서는 구조화 기억 DB 행을 한 번만 읽어 여러 질의 변형에 재사용한다.
  운영 기본 후보 상한은 일반 384행, fallback 1,024행이며, 누적 BLOB 2만여 건을
  배포 중 전체 재색인하지 않는다.
...
보이는 interaction 응답으로 연다. 다른 사용자는 그 버튼을 사용할 수 없다. 서버 메뉴의
학교 공지·편입 공지·개인정보 설정 버튼은 `DM`으로 표시하고 비활성화하며 개인 설정이나
결과를 조회하지 않는다. 비개인 서버 운세 안내와 서버용 기능은 그대로 볼 수 있다.
DM의 `!메뉴`에서는 기존 개인 기능 전체를 사용할 수 있다.
