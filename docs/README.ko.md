...
- 웹 검색은 URL을 정규화하고 같은 동시 질의를 한 요청으로 합친다. 기본 페이지 수집은
  ...
  정확히 한 번 호출한다. 확인된 출처는 봇이 단 📰 반응에 보관하고, 사용자가 📰를
  누르면 같은 메시지에 표시하며 반응을 취소하면 다시 숨긴다.
...
3. 명시적으로 오래된 정보가 필요한 질문은 Discord/Kakao RAG를 확장 검색한다. 의미
   라우터가 사람 이름·과거 사건을 놓치는 회귀를 막기 위해 무도구 일반 대화도 후보
   상한을 낮춘 얕은 검색을 한 번 수행하되, 관련도 임계값을 넘은 기억만 사용한다.
...
- 한 검색 안에서는 구조화 기억 DB 행을 한 번만 읽어 여러 질의 변형에 재사용한다.
  ...
  배포 중 전체 재색인하지 않는다.
- 명시 기억 검색은 구조화 기억과 legacy Discord 원문 임베딩을 함께 비교한다. 일반
  무도구 대화의 얕은 검색은 질의 변형을 1개로 제한하고 구조화 후보가 없을 때만
  legacy 원문을 추가 조회해 TiDB RU와 응답 시간을 제한한다.
- 한국어 이름·고유명사는 조사 제거 lexical 점수를 의미 유사도에 작게 더한다. 특정
  사람 이름을 코드에 하드코딩하지 않으며 서버·채널·사용자/DM mapping 경계는 기존
  SQL 조건 그대로 유지한다.
...
보이는 interaction 응답으로 열고 임시 실행 버튼과 원래 명령 메시지는 권한이 허용되는
범위에서 정리한다. 홈에는 모든 명령을 늘어놓지 않고 `학교·편입`,
...
- 충남대학교와 국립부경대학교는 공식 입학처의 `robots.txt`가 전체 자동 접근을 막아
  자동 구독 대상에 포함하지 않는다. DM 대시보드에 두 공식 공지 링크를 별도 버튼으로
  표시하며, 공식 RSS·API 또는 허용 정책이 생기기 전에는 자동 알림이라고 안내하지 않는다.
...
공개한다. 상세한 20개 자동 공지원, 2개 직접 확인 공식 링크, 개인정보 경계와 운영 절차는
