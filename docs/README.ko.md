...
포함되지만 상주 Discord 프로세스에서는 실행하지 않는다. 별도 one-shot service가 Masamo
전용 core DB와 digest 디렉터리를 사용한다. General은 기본 비활성이며 Masamo의 학교
테이블·파일·timer를 공유하지 않는다.

### 자연어 등록과 확인

DM에서 `!메뉴` 또는 `!공지`의 `학교 설정` 버튼을 누르거나 다음처럼 자연스럽게 말한다.
