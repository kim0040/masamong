# 기억·임베딩 품질과 무삭제 이행 기준

> **문서 성격:** 내부 설계·이행 기록 · **실행 승인 문서 아님**

이 문서는 운영 Masamo의 누적 대화·요약·임베딩을 보존하면서 검색 품질과 구조를 개선할 때
확인할 수 있는 이행 기준입니다. 현재 데이터는 삭제·truncate·전체 재색인하지 않습니다.
General은 별도 빈 DB와 별도 embedding 저장소에서 시작하며 아래 운영 수치를 복사하지
않습니다. 실제 변경은 [운영·배포 가이드](../DEPLOYMENT.md)의 사전 점검과 별도 승인
절차를 함께 만족할 때만 진행합니다.
