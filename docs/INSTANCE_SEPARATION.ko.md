...
이 문서에서는 기존 Masamo 운영 데이터를 유지하면서 새 General 인스턴스를 분리하는 기준을
확인할 수 있습니다. 사용자 기능은 [사용자 가이드](README.ko.md), 일반 배포 절차는
[운영·배포 가이드](DEPLOYMENT.md)를 먼저 확인하세요. 여기의 명령과 경로는 실제 운영
환경에 맞게 검증한 뒤에만 사용해야 합니다.

## 결론

가능합니다. 가장 안전한 구조는 코드를 두 벌로 갈라서 유지하는 대신, 동일한
검증된 release SHA를 두 개의 독립 프로세스로 실행하고 각 프로세스의 정체성·저장소·설정·
로그·스케줄러 소유권을 프로필로 완전히 분리하는 방식입니다.
...
`AI_MAX_CONCURRENT_PROCESSING`, `AI_QUEUE_MAX_SIZE`,
