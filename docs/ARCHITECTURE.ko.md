...
라우터는 같은 JSON 안에서 최종 답변 추론 수준도 `none/low/high`로 고릅니다. 단순
대화·단일 결과 정리에는 `none`, 짧은 비교·설명에는 `low`, 복수 근거·충돌·모호성·
긴 압축 종합에는 `high`를 사용합니다. 난이도 판정을 위한 별도 LLM 호출이나 낮은
단계의 결과를 `high`로 다시 생성하는 경로는 없습니다. 값이 없거나 잘못되면 `none`으로
내려가며, 수준은 공유 target을 수정하지 않고 해당 메시지의 호출 인자로만
...
OpenRouter 경로는 모든 텍스트 요청에 OpenAI 공급자만 허용하고 공급자 fallback을
비활성화합니다. 이미지 생성은 이 경로와 키를 공유하지 않고 기존 CometAPI Gemini
native endpoint를 계속 사용합니다.

최종 system prompt의 마지막에는 모델 독립적인 말투 유지 계약을 둡니다. 공통
정보 전달 규칙보다 현재 채널의 `persona/rules`를 표현 기준으로 삼고, 반말·해요체·
귀여운 말투·호칭을 다른 서버와 섞지 않습니다. 같은 채널의 최근 Bot 답변은 문장
호흡 참고로만 쓰며 사실·수치뿐 아니라 웃음 도배, 반복 문장, 어색한 어미와 오타도
복제하지 않습니다. Discord 전송 직전 정규화는 코드블록을 보존하면서 비정상적으로
긴 `ㅋㅋ/ㅎㅎ`와 한 응답의 반복 표지를 제한하므로 과거 생성 오류가 다시 노출되지
않습니다. 이 과정은 문자열 처리뿐이라 LLM 호출이나 과금을 추가하지 않습니다.
...
사용자 발화뿐 아니라 Discord에 전송을 마친 봇의 최종 답변도
`conversation_history.is_bot=true`로 추가 보존한다. 분할 전송된 답변은
`assistant_response` 한 유닛으로 묶고, 봇이 자신의 선택·취향을 명시한 경우에는
`assistant_commitment`로 표시한다. 이는 당시 입장을 다시 찾기 위한 호환용 내부
분류명이며 이후 답변을 구속하는 약속을 뜻하지 않는다. 생성 시에는 현재 질문, 최근
대화, 새로 확인한 정보가 우선이고 과거 입장은 관련될 때만 참고한다. 이 경로는 LLM
요약을 호출하지 않고 로컬 임베딩 한 번만 예약하며, Discord 이벤트를 다시 발생시키지
않으므로 재귀 응답이나 추가 LLM 과금이 없다.
...
- 명시적 회상에서는 절대 관문·어휘 겹침·상대 `0.72`를 모두 통과한 다른 출처의
  최고 후보를 최대 한 건 보존한다. 일반 잡담에는 적용하지 않는다.
- 개인·어휘 보너스는 순위만 조정하며 절대 의미 관문을 우회하지 못한다.
- Kakao의 `origin`은 저장소 식별자인 `Kakao`로 고정하고 방 이름은
  `source_label`로 분리해, 방 이름이 `UNKNOWN` 출처로 오인되지 않게 한다.
...
정확한 제원·가격·랩타임·성능 기록을 요구하는 비교는 라우터가 도구를 빠뜨려도 웹
검색 한 번을 계획한다. 단순히 “둘 중 네 선택은?”처럼 주관적 선택만 묻고 조회 결과가
없다면 취향과 일반적인 성격 차이만 설명하며 정확한 수치표를 만들지 않는다.
...
취소되면 점유 상태를 즉시 버려 영구 잠금을 막는다. 주식 조회에서는 잘못되거나
모호한 티커를 사용자 입력 실패로 분리해 공급자 회로를 열지 않는다. 현재가 조회가
...
전용 키 또는 공용 CometAPI 키가 없으면 명시적 프로필은 기동하지 않는다. 이미 다른
생성이 진행 중이면 하나의 프로세스 lock에서 설정된 시간까지 기다리며 이 대기는
공급자 호출이나 사용량 예약을 만들지 않는다. HTTP 200인데 이미지가 없는 응답은
자동 재호출하지 않고 candidate 수·종료 사유·안전 분류·파트 수만 기록한다. 생성
텍스트와 프롬프트 원문은 진단 로그에 남기지 않는다.
