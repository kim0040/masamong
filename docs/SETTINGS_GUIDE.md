...
> **시작하기**: [빠른 시작](QUICKSTART.md) | **배포하기**: [운영·배포 가이드](DEPLOYMENT.md) | **문서 전체 보기**: [문서 허브](README.md)

이 가이드에서는 마사몽을 실행하는 데 필요한 환경 변수와 운영 설정을 확인할 수
있습니다. `config.py`는 내부 기본값의 기준이고, `.env.example` 및
`profiles/*.env.example`은 실행 프로필의 예제입니다. 여기서는 운영자가 선택해야 하는
설정만 설명하며, 내부 조정 상수를 모두 나열하지는 않습니다.
...
대화 연속성을 보강합니다. 이 값은 당시 답변을 찾기 위한 내부 분류명이며, 이후 답변을
같은 선택으로 고정하는 규칙이 아닙니다. 현재 질문의 조건, 최근 대화, 새로 확인한
정보가 우선입니다. 기존 행을 수정하거나 삭제하지 않으며, 답변당 로컬 임베딩을 최대
한 번 예약할 뿐 LLM 요약 API는 호출하지 않습니다.
...
| 항목 | 설명 |
|------|------|
| `agent_system_prompt` | 모든 채널에 공통으로 적용할 정보 전달·Discord 형식 규칙 |
| `persona` | AI의 정체성, 말투, 행동 원칙 |
| `rules` | 반드시 지켜야 할 규칙 |
| `allowed` | 해당 채널에서 AI 응답 허용 여부 |

`persona`에는 “다른 채널과 동일하게”처럼 모델이 볼 수 없는 설정을 참조하기보다,
그 채널에서 실제로 사용할 어미·호칭·태도·추임새를 직접 적는 편이 안정적입니다.
공통 정보 형식은 `agent_system_prompt`에 두고, 서버 고유 말투는 각 채널의
`persona`와 `rules`에 둡니다.

런타임은 모델이 바뀌어도 채널 페르소나가 고객센터·교과서식 기본 문체에 눌리지
않도록 공통 말투 유지 계약을 마지막 system 지시로 덧붙입니다. 최근 Bot 답변은
같은 채널의 문체 참고로만 사용하며, 웃음 도배·반복 문장·어색한 어미·오타는 생성
오류로 간주합니다. 최종 Discord 정규화에서도 코드블록 밖 `ㅋㅋ/ㅎㅎ` 연속 도배를
짧게 접고 한 응답의 반복 횟수를 제한합니다.
