...
graph TB
    ...
    subgraph IntentModule["utils/intent_analyzer.py"]
        Analyze["route_tools()<br/>semantic routing"]
        LLMAnalyze["routing lane call<br/>tools, memory, digest"]
        ToolPlan["_sanitize_tool_plan()<br/>contract and caps"]
        Emergency["_emergency_routing_decision()<br/>provider outage fallback"]
    end
    ...
    Analyze --> LLMAnalyze
    LLMAnalyze --> ToolPlan
    LLMAnalyze -. "provider / JSON failure" .-> Emergency
    Emergency --> ToolPlan
    ...
    alt 봇 메시지
        ...
        alt ! 커맨드
            ...
            opt needs_memory=true
                ...
                RAG-->>AI: relevance-gated, source-deduplicated RAG context
            end
...
sequenceDiagram
    ...
    Intent-->>AI: tools: [<br/>  {tool: "get_weather_forecast", params: {location: "Seoul", day_offset: 1}},<br/>  {tool: "web_search", params: {query: "latest Apple stock price"}}<br/>]

    par 날씨 도구 실행
        AI->>Tools: get_weather_forecast(location="Seoul", day_offset=1)
    ...
    and Ground current finance facts
        AI->>Tools: web_search(query="latest Apple stock price")
...
flowchart TD
    ...
    CheckMemory -->|Yes| RAGSearch[Selective long-term memory<br/>RAGManager.search]
    CheckMemory -->|No| BuildPrompt[Build prompt<br/>persona + tool results + digest + recent verbatim]
...
```mermaid
flowchart TD
    Start([Semantic routing starts]) --> BuildPrompt[Tool contract + request + recent turns]
    BuildPrompt --> NeedDigest{Older section over<br/>compaction threshold?}
    NeedDigest -->|Yes| AddDigest[Include compaction source in same call]
    ...
    Emergency --> Decision
    Decision --> Done([Routing complete])
```
