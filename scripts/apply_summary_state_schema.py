#!/usr/bin/env python3
"""채널 증분 요약 상태 테이블 하나를 additive 생성합니다."""
...
_CREATE_RE = re.compile(
    r"\ACREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+channel_summary_state\s*\(",
    re.IGNORECASE,
)
...
def main() -> int:
    ...
    schema_statement(config.DB_BACKEND)
    if not args.apply:
        print("DRY-RUN: 기존 행을 읽거나 수정하지 않고 CREATE TABLE 1개만 준비합니다.")
        print(phrase)
        return 0
    asyncio.run(apply_schema())
    print("채널 요약 상태 schema 적용과 컬럼 검증이 완료되었습니다.")
    return 0
