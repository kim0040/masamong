...
from cogs.admin_cog import AdminCog, AdminLauncherView
from cogs.commands import UserCommands
from cogs.fun_cog import FunCog
from cogs.help_cog import HelpCog, MasamongHelpCommand, ServerMenuLauncherView
...
class DummyChannel:
    ...
    async def send(self, content: str | None = None, *, embed: Any = None, file: Any = None, view: Any = None, **kwargs) -> DummySentMessage:
        msg = DummySentMessage(content=content, embed=embed, file=file, view=view)
...
class DummyBot:
    def __init__(self):
        ...
        self.user = SimpleNamespace(display_name="masamong", avatar=None, id=999)
        self.db = object()
        self.guilds = []
        self.latency = 0.01
...
async def case_server_private_menu_launcher() -> None:
    bot = DummyBot()
    guild = DummyGuild(6)
    channel = DummyChannel(602, guild=guild)
    ctx = DummyContext(channel=channel, author=DummyAuthor(21), guild=guild)
    dummy_cog = SimpleNamespace(bot=bot)

    await HelpCog.menu.callback(dummy_cog, ctx)
...
async def case_admin_private_panel() -> None:
    bot = DummyBot()
    guild = DummyGuild(6)
    channel = DummyChannel(603, guild=guild)
    author = DummyAuthor(22)
    ctx = DummyContext(channel=channel, author=author, guild=guild)
    previous = config.SUPERADMIN_USER_IDS
    config.SUPERADMIN_USER_IDS = frozenset({author.id})
    try:
        cog = AdminCog(bot)
        await cog.admin.callback(cog, ctx)
    finally:
        config.SUPERADMIN_USER_IDS = previous

    message = channel.messages[-1]
    assert message.embed is not None
    assert isinstance(message.view, AdminLauncherView)
...
async def case_delete_log() -> None:
    ...
    previous_admins = config.SUPERADMIN_USER_IDS
    config.LOG_FILE_NAME = temp_path
    config.SUPERADMIN_USER_IDS = frozenset({ctx.author.id})
    try:
        await cog.delete_log.callback(cog, ctx)
    finally:
        config.LOG_FILE_NAME = prev
        config.SUPERADMIN_USER_IDS = previous_admins
...
async def main() -> int:
    cases: list[tuple[str, Callable[[], Coroutine[Any, Any, None]]]] = [
        ...
        ("server_private_menu", case_server_private_menu_launcher),
        ("admin_private_panel", case_admin_private_panel),
