#!/usr/bin/env python3
"""프로필별 봇 관리자 계정 테이블 하나를 additive 생성합니다."""
...
_CREATE_RE = re.compile(
    r"\ACREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+bot_admin_accounts\s*\(",
    re.IGNORECASE,
)
...
def confirmation_phrase() -> str:
    return (
        "APPLY ADMIN ACCOUNTS SCHEMA TO "
...
def main() -> int:
    ...
    print("인스턴스 관리자 schema 적용과 컬럼 검증이 완료되었습니다.")
    return 0
